...
def catch_parameters():
    ...
    # Tests showed that the address returned by classic is usually used as a CALLER
    # and the addressed used by normal STUN when being the CALLEE
    if CALL_MODE == 'CALLER':
        # CLASSICSTUN
        stun_byte_sequence = b'\x01\x15'
        attribute_byte_sequence = b'\x00\x12'
    elif CALL_MODE == 'CALLEE':
        # STUN
    ...
    stage2_bpf_filter = "ip and ip dst host {own_ip} and ip src host {skype_ip} " \
                        "and udp and (udp src port 3478 or udp src port 3480)".format(own_ip=OWN_IP,
                                                                                    skype_ip=SKYPE_IP)
...
if __name__ == "__main__":
    ...
    def tunnel_receiver(interface):
        ...
        while True:
            ...
            interface.write(data[28 + len(IDENTIFIER):])
    ...
    # Ethernet header length 14 bytes, IP header has a max length of 24 Bytes and UDP header 8 bytes
    # magic 4 bytes from the tunnel interface
    tun.mtu = MTU - 14 - 20 - 8 - len(IDENTIFIER) - 4
    ...
    iptables_cmd = 'sudo iptables -I FORWARD  -d {dst} -s {src} -p udp --sport {sport} -m u32 --u32' \
                   ' "28=0xcafebabe" -j DROP'.format(dst=NAT_IP, src=SKYPE_IP, sport=SKYPE_PORT)
    ...
    def sender():
        ...
        if SEND_SCAPY:
            ...
            templatePktraw = templatePkt.build() + IDENTIFIER
